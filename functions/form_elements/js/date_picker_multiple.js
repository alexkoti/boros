...
/**
 * Calendário com multiseleção de datas
 * @link http://foxrunsoftware.github.io/DatePicker/
 * 
 * 
 */
jQuery(document).ready(function($){
	...
	if( $('.date_picker_multiple_calendars').length ){
		
		// inicializar os calendários
		$('.date_picker_multiple_calendars').each(function(){
			var selected_dates = $(this).closest('.date_picker_multiple_box').find('.date_picker_input').val().split(','); //console.log(selected_dates);
			// pegar a quantidade de calendários a ser mostrada
			var num_months = Number( $(this).attr('data-num-months') );
			var id = $(this).attr('id');
			
			$('#'+id).DatePicker({
				inline: true,
				mode: 'multiple',
				calendars: num_months,
				next: ' &gt; ',
				prev: ' &lt; ',
				date: selected_dates,
				onChange: function(date, el) {
					...
					for( var i = 0; i < dlength; i++ ){
						// validar data, pois quando se usa o mode: 'multiple', date, sempre retorna a primeira data do array como inválida
						if( isValidDate(date[i]) ){
							var fdate = new Date(date[i]);
							//v.push( fdate.getFullYear() + '-' + (fdate.getMonth()+1) + '-' + fdate.getDate() );
							v.push( fdate.getFullYear() + '-' + ('0' + (fdate.getMonth()+1)).slice(-2) + '-' + ('0' + fdate.getDate()).slice(-2) + ' 00:00:00' );
						}
					}
					$(el).prev('input').val( v );
				},
		...
		// enviar as datas para o input:hidden
		//$('.date_picker_multiple_box').each(function(){
		//	var selected_dates = $(this).find('.date_picker_input').val().split(','); console.log(selected_dates);
		//	$(this).find('.date_picker_multiple_calendars').DatePickerSetDate( selected_dates );
		//});
	}
	
	/**
	 * Detectar data em formato válido
	 * @link http://stackoverflow.com/q/1353684
	 * 
	 */
