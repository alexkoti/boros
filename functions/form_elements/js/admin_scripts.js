	...
	$(document).ajaxSend(function(e, xhr, settings) {
		...
		if( action == 'save-widget' ){
			...
			if( $('#widget-' + widget_id + '-html').length && $('#widget-' + widget_id + '-html').is('.form_text_editor') ){
				// corrigir altura do textarea original = altura do tiny_mce aplicado
				$('#widget-' + widget_id + '-html').css(
					'height',
...
/**
 * TINYMCE EM DUPLICATE ELEMENTS
 * É preciso monitorar o momento da duplicação do elemento, para iniciar o tinymce após o loading e também o sort dos elementos, para que o content visível seja atualizado.
 * 
 * 
 */
jQuery(document).ready(function($){
	
	$('.duplicate_element').bind('duplicate_item_complete', function(event, id){
		//console.log(event);
		//console.log(id);
	...
	$('.duplicate_group').bind( "sortstart", function(event, ui){
		...
		var textarea = $('#'+ui_id).find('.form_textarea_editor');
		if(textarea.length){
			//console.log(ui);
			//console.log(ui_id);
...
function update_widget_tinymce_contents(){
	jQuery('.free_box_form, #addtag #submit').closest('form').unbind('click').bind('mousedown', function(){
